...
try
{
    ...
    await using var container = new OracleBuilder("gvenzl/oracle-free:23-slim-faststart")
        .WithName("OracleDependency")
        .WithWaitStrategy(Wait.ForUnixContainer().UntilDatabaseIsAvailable(OracleClientFactory.Instance))
    ...
    await executor.ExecuteNonQueryAsync("create table if not exists dept (deptno number generated by default on null as identity, dname varchar2(14), loc varchar2(13), constraint pk_dept primary key (deptno))", cts.Token);
